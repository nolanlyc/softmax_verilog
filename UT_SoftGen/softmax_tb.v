...
i=0;
  ...
  f = $fopen("output.txt","w");
   ...
   for (q=0; q<1000; q=q+1) begin
     #4  ;
     if(softmax.mode7_run == 1) begin
        i=i+1;
	$display("========= set %d",i);
	$display("Final out3 %d",softmax.mode7_exp.outp3);
	$display("Final out2 %d",softmax.mode7_exp.outp2);
	$display("Final out1 %d",softmax.mode7_exp.outp1);
	$display("Final out0 %d",softmax.mode7_exp.outp0);
    	$fwrite(f,"%d\n",softmax.mode7_exp.outp3);
    	$fwrite(f,"%d\n",softmax.mode7_exp.outp2);
    	$fwrite(f,"%d\n",softmax.mode7_exp.outp1);
    	$fwrite(f,"%d\n",softmax.mode7_exp.outp0);
